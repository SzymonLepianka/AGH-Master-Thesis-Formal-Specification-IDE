...
class XmlParserServiceTest {
    ...
    @Test
    void parseXmlPaymentGenGenMyModel() throws Exception {
        var project = modelFactory.createProject("test24");
        var useCaseDiagram = modelFactory.createUseCaseDiagram(project, UUID.randomUUID());
        xmlParserService.parseXml(useCaseDiagram, new File("xml_and_png_examples/1. GenMyModel/payment_gen.xml"));
        var useCaseList = useCaseDiagram.getUseCaseList();
        for (var useCase : useCaseList) {
            var useCaseName = useCase.getUseCaseName();
            var useCasePrettyName = useCase.getUseCasePrettyName();
    ...
    @Test
    void parseXmlPaymentGenPapyrus() throws Exception {
        var project = modelFactory.createProject("test25");
        var useCaseDiagram = modelFactory.createUseCaseDiagram(project, UUID.randomUUID());
        xmlParserService.parseXml(useCaseDiagram, new File("xml_and_png_examples/2. Papyrus/Payment_gen.uml"));
        var useCaseList = useCaseDiagram.getUseCaseList();
        for (var useCase : useCaseList) {
            var useCaseName = useCase.getUseCaseName();
            var useCasePrettyName = useCase.getUseCasePrettyName();
